...
class NewPostViewController: UIViewController {
    ...
    var categoryName =  ["Grocery", "Work", "Sport", "Design", "University", "Social", "Music", "Health", "Movie", "Home", "Create New"]
...
//        let tapToDismiss = UITapGestureRecognizer(target: self, action: #selector(tapResignKeyboard))
...
//        view.bringSubviewToFront(categoryView)
    ...
    @IBAction func saveChooseTime(_ sender: Any) {
        ...
        let selectedTime = timePickerView.date
            
            let formatter = DateFormatter()
            formatter.dateFormat = "yyyy-MM-dd hh:mm a"
            let formattedTime = formatter.string(from: selectedTime)
            print("Selected time with AM/PM: \(formattedTime)")
        isEnableAllSubviews(enable: true)
        titleTextField.becomeFirstResponder()
    }
